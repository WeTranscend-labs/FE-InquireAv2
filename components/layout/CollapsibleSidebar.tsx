...
export function CollapsibleSidebar() {
  ...
  const getNextMilestone = (rep) => {
    ...
    return milestones.find(m => m > rep) || milestones[milestones.length - 1] * 2;
  };

  const nextMilestone = getNextMilestone(reputationValue);
  const progressPercentage = Math.min(100, (reputationValue / nextMilestone) * 100);
  ...
  const addressFirstLetter = address ? address.substring(2, 3).toUpperCase() : '?';
            ...
            {!isCollapsed ? (
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="font-semibold text-sm">
                        {isUserLoading ? 'Loading...' : `${reputationValue.toLocaleString()}`}
                      </span>
                    </div>
              ...
              <TooltipProvider delayDuration={200}>
                <Tooltip>
                  ...
                  <TooltipContent side="right" className="flex flex-col gap-2 p-3 w-48">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-xs font-medium">
                        {isBalanceLoading ? '...' : `${balance} ${chains[0]?.nativeCurrency?.symbol ?? ''}`}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-xs">Reputation:</span>
                      <span className="text-xs font-medium">
                        {isUserLoading ? '...' : reputationValue.toLocaleString()}
                      </span>
                    </div>
                  </TooltipContent>
                </Tooltip>
            ...
            <nav className="space-y-1.5">
              {sidebarItems.map((item) => {
                ...
                return isCollapsed ? (
                  ...
                  >
                    <Link href={item.href}>
                      <item.icon className={cn("h-4 w-4 mr-3", isActive && "text-primary")} />
